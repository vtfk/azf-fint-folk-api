...
const { repackNavn, aktivPeriode, repackAdresselinje, createStruktur, getAge } = require('./repack-fint')
const { fintGraph } = require('./call-fint')
const { getUserFromAnsattnummer } = require('./call-graph')
...
const repackEmployee = (fintEmployee) => {
  ...
  const employee = {
    ...
    kjonn: fintEmployee.personalressurs.person.kjonn.kode,
...
module.exports = async (ansattnummer) => {
  ...
  logger('info', ['fintEmployee', 'Repacked result - fetching AzureAd info', 'ansattnummer', ansattnummer])
  const aad = await getUserFromAnsattnummer(ansattnummer)
  if (aad.value && aad.value.length === 1) {
    logger('info', ['fintEmployee', 'Found user in AzureAd', 'ansattnummer', ansattnummer])
    ...
    logger('info', ['fintEmployee', 'Could not find user in AzureAd', 'ansattnummer', ansattnummer])
  }
