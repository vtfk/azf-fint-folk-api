...
const getNarmesteLeder = (ansattnummer, strukturlinje) => {
  ...
  const assumedLeader = strukturlinje.find(enhet => enhet.leder && (enhet.leder.ansattnummer !== ansattnummer)) || null // Finn første enhet som har en leder, som ikke er den ansatte
  const repackedAssumedLeader = repackLeder(assumedLeader.leder)
  return repackedAssumedLeader
}
