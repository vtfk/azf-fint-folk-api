const aktivPeriode = (periode) => {
  ...
  const actualPeriode = {
    start: periode.start ? new Date(periode.start.slice(0, 10)) : null,
    slutt: periode.slutt ? new Date(periode.slutt.slice(0, 10)) : null
  }
  if (actualPeriode.start.toString() === 'Invalid Date') return false // Hvis startdato ikke er gyldig får vi anta at det ikke er aktivt
  if (now > actualPeriode.start) { // Sjekk først om perioden er senere enn start
    if (!actualPeriode.slutt) return true // Hvis ikke det finnes noen slutt er vi good
    if (actualPeriode.slutt.toString() === 'Invalid Date') return true // Hvis sluttdato ikke er gyldig får vi anta at vi er good
    if (now < actualPeriode.slutt) return true // Hvis sluttdato ikke er nådd enda er vi også good
  }
...
const repackGyldighetsperiode = (gyldighetsperiode) => {
  const newGyldighetsperiode = { ...gyldighetsperiode }
  
  if (newGyldighetsperiode.start) {
    newGyldighetsperiode.start = "" // fiks date til xxxx-xx-xxT00:00:00
  }
  
  if (newGyldighetsperiode.slutt) {
    newGyldighetsperiode.slutt = "" // fiks date til xxxx-xx-xxT23:59:59
  }
  
  return newGyldighetsperiode
}
