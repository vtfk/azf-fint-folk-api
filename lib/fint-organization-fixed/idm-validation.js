...
const validateRawOrganizationUnits = (rawOrganizationUnits, context) => {
  const validationResult = {
    ...
    tests: {
      ...
      childDoesNotExist: {
        description: 'Enheter som har underordnet som ikke finnes (eller har duplikat)',
        data: 'not run'
      },
      brokenParentRelation: {
        description: 'Enheter med relasjon til en overordnet som enten ikke finnes, eller der overordnet ikke har underordnet som peker tilbake',
        data: 'not run'
      },
  ...
  try {
    ...
    validationResult.tests.childDoesNotExist.data = enheter.filter(enhet => Array.isArray(enhet._links.underordnet) && enhet._links.underordnet.length > 0 && enhet._links.underordnet.some(link => {
      try {
        getUnitFromLink(link.href, enheter)
        return false
      } catch (error) {
        return true // Child not found
      }
    }))
    ...
    enheter = enheter.filter(enhet => !validationResult.tests.childDoesNotExist.data.some(unit => unit.organisasjonsId.identifikatorverdi === enhet.organisasjonsId.identifikatorverdi)) // Fjern enheter med barn som ikke finnes
    ...
    validationResult.valid = validationResult.tests.allowedNumberOfUnits.data &&
      ...
      (Array.isArray(validationResult.tests.childDoesNotExist.data) && validationResult.tests.childDoesNotExist.data.length === 0) &&
