...
const validateRawOrganizationUnits = (rawOrganizationUnits, context) => {
  const validationResult = {
    ...
    tests: {
      ...
      topUnits: {
        description: 'Topenheter (enheter som har seg selv som overordnet) - må ha minst en',
        data: 'not run'
      }
    },
    validUnits: null
...
const validateExceptionRules = (exceptionRules, preValidatedUnits, context) => {
  ...
  let { overrideNextProbableLink, absorbChildrenOverrides, allowNoCorrespondingBottomUnit, manualLeaders } = exceptionRules
  const validationResult = {
    valid: false,
    internalError: 'no error',
    invalidRules: []
