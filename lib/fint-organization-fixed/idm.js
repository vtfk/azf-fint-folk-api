...
const repackFintIdmEnheter = (topUnits, validatedUnits, aditroUnits, exceptionRules, context) => {
  ...
  const repackFintEnhetRecursive = (unit) => {
    ...
    const organisasjonsType = {
      systenId: aditroProjectDimension6HoursResult.projectDimension6Hours.id,
  ...
  try {
    ...
    validation.tests.missingAditroOrgType.status = validation.tests.missingAditroOrgType.data.length < maxUnitsWithUnknownOrgType ? 'pass' : 'warn'
