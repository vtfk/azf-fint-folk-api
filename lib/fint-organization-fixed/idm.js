...
const repackFintIdmEnheter = (topUnits, validatedUnits, exceptionRules, context) => {
  ...
  const repackFintEnhetRecursive = (unit) => {
    ...
    if (abstractTraversedUnitsNameChain.some(name => name.toLowerCase() !== correspondingBottomUnit.navn.toLowerCase())) {
      ...
      if (!vgsNameChain(abstractTraversedUnitsNameChain)) { // If not vgs special case, we have to check more
        ...
        if (nameChainOverrideRule && (nameChainOverrideRule.allowedNameChain.join(',').toLowerCase() === abstractTraversedUnitsNameChain.join(',').toLowerCase())) {
          ...
          validation.tests.correspondingBottomUnitDifferentName.data.push({ organisasjonsId: unitCopy.organisasjonsId, navn: unitCopy.navn, abstractTraversedUnits })
        }
      }
    }
    ...
    if (!willBeAbsorbed) {
      resultingUnitsFlat.push(repackedFintUnit)
    } else {
  ...
  try {
    ...
    writeFileSync('./ignore/resultingUnitsFlat.json', JSON.stringify(resultingUnitsFlat, null, 2))
    writeFileSync('./ignore/resultingUnitsNested.json', JSON.stringify(resultingUnitsNested, null, 2))
    writeFileSync('./ignore/validationResult.json', JSON.stringify(validation, null, 2))

    // Check that valid and return with units if ok
    validation.valid = validation.tests.correspondingBottomUnitMissing.data.length === 0 &&
      validation.tests.correspondingBottomUnitDifferentName.data.length === 0 &&
      validation.tests.notHandledUnits.data.length === 0 &&
      validation.tests.handledMoreThanOneTimeUnits.data.length === 0
...
const fintOrganizationFixedIdm = async (context) => {
  ...
  const repackedFintUnitsResult = repackFintIdmEnheter(rawValidationResult.tests.topUnits.data, rawValidationResult.validUnits, exceptionRules, context)
