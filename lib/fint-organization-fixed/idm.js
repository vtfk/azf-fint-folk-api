...
const repackFintIdmEnheter = (topUnits, validatedUnits, exceptionRules, context) => {
  const { absorbChildrenOverrides, overrideNextProbableLink, allowNoCorrespondingBottomUnit, manualLeaders } = exceptionRules // Desctructure for easier access
  const handledUnits = []
  ...
  const validation = {
    ...
    tests: {
      ...
      correspondingBottomUnitDifferentName: {
        description: 'Enheter der navnet på tilsvarende enhet på bunnivå er ikke matcher navnet på enheten',
        data: []
      },
      notHandledUnits: {
  ...
  const repackFintEnhetRecursive = (unit) => {
    ...
    while (Array.isArray(correspondingBottomUnit._links.underordnet) && correspondingBottomUnit._links.underordnet.length > 0) { // As long as there are underordnet, we keep going down until bottom level
      ...
      // First we check if this unit has an override rule for next link - in that case, we use that instead of checking for pattern nextLink
      ...
      // Check that the nextProbableLink actually exists in underordnet, if not, this unit is not on a valid format
      if (!correspondingBottomUnit._links.underordnet.some(link => link.href === nextLevelProbableLink)) {
        /* Naaah, this part is to complicated to understand - use rules instead
        // In some special cases, it's ok if the nextProbableLink is missing - but ONLY if if the current unit is going to be absorbed by the parent, because then all the data we would have found on the correspondingBottomLevelUnit would be replaced by the parent anyway
        const willBeAbsorbed = willBeAbsorbedBy(unitCopy)
        if (willBeAbsorbed) {
          logger('warn', [`NextProbableLink missing for ${unitCopy.navn} (${unitCopy.organisasjonsId.identifikatorverdi}) - but SPECIAL CASE. The unit will be absorbed by parent ${willBeAbsorbed[0]} later on, and we are ok for now...`], context)
        } else {
    ...
    // Then we validate that the correspondingBottomUnit has the same name as the unit we are currently on - we probably need exception rules here as well
    if (correspondingBottomUnit.navn !== unitCopy.navn) { // TODO - vurdere om hele navne-chainen skal stemme? Evt. at det er nok at det siste leddet stemmer?
      if (!(correspondingBottomUnit.navn.toLowerCase().endsWith('vgs') && (unit.navn.toLowerCase().endsWith('videregående skole') || unit.navn.toLowerCase().endsWith('videregåande skule')))) { // Special case for VGS, where we allow the name to be different
        validation.tests.correspondingBottomUnitDifferentName.data.push({ unit, differentName: { navn: correspondingBottomUnit.navn, organisasjonsId: correspondingBottomUnit.organisasjonsId.identifikatorverdi } })
      }
    }
    ...
    // Build the actual unit TODO -build it better...
    const actualUnit = {
    ...
    const repackedFintUnit = {
      ...
      abstractTraversedLinks: nextLevelProbableLinks,
    ...
    // And then maybe we should do some fancy checks to get correct leader? If we are at top level - set leader to leader of FD or something. If not top level, set leader to leader of parent? And so on...

    // Ikke legg til absorbed enheter i resultatet!
    const willBeAbsorbed = willBeAbsorbedBy(unitCopy)
    if (!willBeAbsorbed) {
...
const fintOrganizationFixedIdm = async (context) => {
  ...
  if (!rawValidationResult.valid) throw new Error('Prevalidation of raw organization units failed, please run validate endpoint to see what went wrong')
...
module.exports = { fintOrganizationFixedIdm }
