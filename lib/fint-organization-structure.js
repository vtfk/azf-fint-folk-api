...
const repackOrganizationStructure = (unit, includeInactiveUnits, counter) => {
  ...
  unit.underordnet = unit.underordnet.map(u => repackOrganizationStructure(u, includeInactiveUnits, counter))
...
const fintOrganizationStructure = async (includeInactiveUnits) => {
  ...
  const repacked = repackOrganizationStructure(data.organisasjonselement, includeInactiveUnits, counter) // Modifies the object directly, but we return object in question because js, and easier to understand
