...
const { skoleElementIsAktiv, repackNavn, repackSkolear, repackTermin, repackAdresselinje, repackSkole, repackMiniSkole, getAge, aktivPeriode } = require('./repack-fint')
const { fintGraph } = require('./call-fint')
const { getUserFromSamAccount } = require('./call-graph')
...
module.exports = async (feidenavn) => {
  logger('info', ['fintTeacher', 'Creating graph payload', 'feidenavn', feidenavn])
  const payload = graphQlTeacher(feidenavn)
  logger('info', ['fintTeacher', 'Created graph payload, sending request to FINT', 'feidenavn', feidenavn])
  ...
  logger('info', ['fintTeacher', 'Repacked result - fetching AzureAd info', 'feidenavn', feidenavn])
  const aad = await getUserFromSamAccount(feidenavn.split('@')[0])
  if (aad.value && aad.value.length === 1) {
    logger('info', ['fintTeacher', 'Found user in AzureAd', 'feidenavn', feidenavn])
    ...
    logger('info', ['fintTeacher', 'Could not find user in AzureAd', 'feidenavn', feidenavn])
  }
