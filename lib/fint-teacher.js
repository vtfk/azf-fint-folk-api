...
const fintTeacher = async (feidenavn, context) => {
  logger('info', ['fintTeacher', 'Creating graph payload', 'feidenavn', feidenavn], context)
  const payload = graphQlTeacher(feidenavn)
  logger('info', ['fintTeacher', 'Created graph payload, sending request to FINT', 'feidenavn', feidenavn], context)
  ...
  logger('info', ['fintTeacher', 'Repacked result - fetching AzureAd info', 'feidenavn', feidenavn], context)
  const aad = await getUserFromSamAccount(feidenavn.split('@')[0], context)
  if (aad.value && aad.value.length === 1) {
    logger('info', ['fintTeacher', 'Found user in AzureAd', 'feidenavn', feidenavn], context)
    ...
    logger('info', ['fintTeacher', 'Could not find user in AzureAd', 'feidenavn', feidenavn], context)
  }
